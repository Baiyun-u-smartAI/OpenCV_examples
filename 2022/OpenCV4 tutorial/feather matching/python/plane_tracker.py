...
class PlaneTracker:
    def __init__(self):
        self.detector = cv.AKAZE_create() # 如果使用AKAZE特征采用AKAZE_create()或者BRISK特征：BRISK_create()
    ...
    def add_target(self, image, rect, data=None):
        ...
        x0, y0, x1, y1 = rect
        raw_points, raw_descrs = self.detect_features(image) # 对image计算特征
        points, descs = [], []
        for kp, desc in zip(raw_points, raw_descrs):
            x, y = kp.pt
            if x0 <= x and x <= x1 and y0 <= y and y <= y1: # 所选区域内的关键点和描述符添加到points, descs
                points.append(kp)
                descs.append(desc)
        descs = np.uint8(descs)
        self.matcher.add([descs])
        target = PlanarTarget(image = image, rect=rect, keypoints = points, descrs=descs, data=data) # PlanarTarget是一个组元
